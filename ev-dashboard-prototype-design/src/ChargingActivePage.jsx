...
function tk(theme) {
  ...
  return {
    pageBg:       dark ? "linear-gradient(135deg,#0a1a0f 0%,#0f1520 50%,#130f1f 100%)" : "linear-gradient(135deg,#e8f5e9 0%,#f0f9ff 50%,#faf5ff 100%)",
  ...
  return (
    <div style={{ position: "absolute", width: 350, height: 350, left: 48, top: 65 }}>
      <svg width="350" height="350" viewBox="0 0 350 350">
        ...
        {/* Track */}
        <path d={arcPath(startAngle, sweepAngle, r)} fill="none"
          stroke={t.trackColor} strokeWidth={sw} strokeLinecap="round" />

        {/* Filled arc */}
        {filledSweep > 0 && (
          <path d={arcPath(startAngle, filledSweep, r)} fill="none"
        ...
        <circle cx={cx} cy={cy} r={r + sw / 2 + 5} fill="none"
          stroke={fillColor} strokeWidth="1" opacity="0.12" />
      </svg>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "flex-start" }}>
          <span style={{ fontFamily: FONT, fontWeight: 800, fontSize: 108, color: t.gaugeText, lineHeight: "130px" }}>{displayed}</span>
          <span style={{ fontFamily: FONT, fontWeight: 700, fontSize: 36, color: t.gaugeText, marginTop: 24 }}>%</span>
        </div>
        <span style={{ fontFamily: FONT, fontWeight: 400, fontSize: 28, color: t.textSecond, marginTop: -12 }}>Battery</span>
...
function InfoCard({ label, value, sub, icon, bg, style, theme }) {
  ...
  return (
    <div style={{
      background: t.cardBg, borderRadius: 20, padding: "16px 22px",
      boxShadow: t.cardShadow, border: t.cardBorder,
      display: "flex", alignItems: "center", gap: 18, ...style,
    }}>
      <div style={{
        width: 60, height: 60, background: bg,
      ...
      <div>
        <div style={{ fontFamily: FONT, fontWeight: 400, fontSize: 14, color: t.textSecond }}>{label}</div>
        <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 30, color: t.textPrimary, lineHeight: "36px" }}>{value}</div>
        {sub && <div style={{ fontFamily: FONT, fontWeight: 400, fontSize: 13, color: t.textMuted }}>{sub}</div>}
      </div>
    </div>
  );
}
...
function StopButton({ onStop }) {
  ...
  return (
    <button
      ...
      style={{
        width: "100%", height: 72, background: hovered ? "#CC0000" : RED,
...
/* ── Page Root ───────────────────────────────────────────────────── */
export default function ChargingActivePage({ battery = 65, navActive, setNavActive, theme = "light" }) {
  const t = tk(theme);
  ...
  return (
    <div style={{
      ...
      {/* Ambient blobs */}
      <div style={{ position: "absolute", width: 500, height: 500, left: -60, top: 80, borderRadius: "50%", background: `radial-gradient(circle,${t.blob1} 0%,transparent 70%)`, pointerEvents: "none", zIndex: 1 }} />
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 1 }}>
        ...
        {/* Right panel */}
        <div style={{ position: "absolute", left: 900, top: 80, width: 356 }}>
          <div style={{ fontFamily: FONT, fontWeight:600, fontSize:20, letterSpacing:"1.2px", textTransform:"uppercase", color:t.textSecond, marginBottom: 16 }}>
            Charging Status
          </div>

          <InfoCard
            label="Charging Rate" bg={t.iconBg.green}
            value={<><span style={{ color: t.textPrimary }}>75 </span><span style={{ fontSize: 20, fontWeight: 400, color: t.textSecond }}>kW</span></>}
            sub="Fast Charging"
          ...
          <InfoCard
            label="Duration" bg={t.iconBg.blue}
            value={<><span style={{ color: t.textPrimary }}>1</span><span style={{ fontSize: 20, fontWeight: 400, color: t.textSecond }}> h </span><span style={{ color: t.textPrimary }}>05</span><span style={{ fontSize: 20, fontWeight: 400, color: t.textSecond }}>m</span></>}
            icon={
          ...
          <InfoCard
            label="Estimated Full" bg={t.iconBg.purple}
          ...
          {!stopped
            ...
            : (
              <div style={{
                width: "100%", height: 72, background: t.stoppedBg,
                borderRadius: 16, display: "flex", alignItems: "center",
                justifyContent: "center", gap: 10,
              }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={t.stoppedText} strokeWidth="2.5" strokeLinecap="round">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span style={{ fontFamily: FONT, fontWeight: 600, fontSize: 18, color: t.stoppedText }}>Charging Stopped</span>
              </div>
            )
          }
          ...
          {!stopped && (
            <div style={{
              marginTop: 12, background: t.hintBg,
              ...
              <span style={{ fontFamily: FONT, fontSize: 13, color: theme === "dark" ? "#4ADE80" : GREEN_DARK, fontWeight: 500 }}>
