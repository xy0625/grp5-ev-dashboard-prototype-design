...
function MapArea() {
  return (
    <div
      ...
      {/* Speed circle */}
      <div style={{ position:"absolute", width:145, height:145, left:18, top:431, background:"#fff", boxShadow:"0px 4px 4px rgba(0,0,0,0.25)", borderRadius:"50%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
      ...
      {/* Max speed */}
      <div style={{ position:"absolute", width:75, height:75, left:183, top:473, background:"#fff", boxShadow:"0px 6px 20px rgba(0,0,0,0.1)", borderRadius:25, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
        <div style={{ fontFamily:"Inter", fontWeight:600, fontSize:16, color:"rgba(128,128,128,0.8)" }}>MAX</div>
        <div style={{ fontFamily:"Inter", fontWeight:700, fontSize:24, color:"rgba(0,0,0,0.8)" }}>90</div>
      </div>
    </div>
  );
}
...
function ClimateCard({ temp, setTemp, setActive }) {
  ...
  return (
    <div style={{ position: "absolute", width: 356, height: 142, left: 512, top: 300, background: "rgba(255,255,255,0.9)", boxShadow: "0px 6px 20px rgba(0,0,0,0.1)", borderRadius: 20 }}>
      ...
      {/* Fan block */}
      <div style={{ position: "absolute", left: 26, top: 78, width: 80, height: 46, background: "#fff", border: "1px solid #D9D9D9", borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
        ...
        <div style={{ display: "flex", gap: 3, alignItems: "flex-end" }}>
          {bars.map((h, i) => (<div key={i} onClick={() => toggleBar(i)} style={{ width: 4, height: h, borderRadius: 2, cursor: i === 0 ? "default" : "pointer", background: activeBars[i] ? "#0096FF" : "#D9D9D9", transition: "0.2s", opacity: i === 0 ? 1 : 0.9,}}/>))}</div>
      </div>

      {/* Temp control */}
      <div style={{ position:"absolute", right:20, top:72, display:"flex", alignItems:"center", gap:10 }}>
        <button onClick={() => setTemp(t => Math.max(16, t-1))} style={{ width:44, height:44, border:"1px solid #D9D9D9", borderRadius:20, background:"#fff", fontSize:20, cursor:"pointer" }}>−</button>
        <span style={{ fontFamily:"Inter", fontWeight:500, fontSize:20, minWidth:50, textAlign:"center" }}>{temp}°</span>
        <button onClick={() => setTemp(t => Math.min(30, t+1))} style={{ width:44, height:44, border:"1px solid #D9D9D9", borderRadius:20, background:"#fff", fontSize:20, cursor:"pointer" }}>+</button>
      </div>
    </div>
  );
}
...
function TripCard({ setActive }) {
  return (
    <div style={{ position:"absolute", width:356, height:180, left:512, top:470, background:"rgba(255,255,255,0.95)", boxShadow:"0px 6px 20px rgba(0,0,0,0.1)", borderRadius:20 }}>
      ...
      <div style={{ position:"absolute", width:40, height:40, left:21, top:57, background:"#E8F9EF", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      {[
        ...
        <div key={label} style={{ position:"absolute", width:92, height:56, left, top:107, background: accent?"rgba(243,237,255,0.8)":"#fff", border:"1px solid #EAEAEA", boxShadow:"0px 6px 20px rgba(0,0,0,0.1)", borderRadius:16 }}>
  ...
  return (
    <div style={{ position:"absolute", width:332, height:164, left:897, top:64, background:"#fff", boxShadow:"0px 6px 20px rgba(0,0,0,0.1)", borderRadius:20 }}>
      ...
      <div style={{ position:"absolute", left:33, top:120, display:"flex", gap:14 }}>
        {days.map((d,i) => (
          <div key={i} style={{ width:25, height:27, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, border: i===dow?"2px solid #0078FF":"none", background: i===dow?"rgba(245,245,245,0.5)":"transparent", fontFamily:"Roboto", fontWeight:500, fontSize:14, color: i===dow?"#0078FF":"#B0A2A2" }}>{d}</div>
        ))}
      </div>
    </div>
  );
