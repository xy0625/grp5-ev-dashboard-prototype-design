...
function tk(theme) {
  ...
  return {
    pageBg:       dark ? "#0F1117" : "linear-gradient(155deg, #EEF4FA 0%, #DFF0F8 30%, #D0E8F4 55%, #C8E0F0 75%, #D8ECD8 90%, #EEF6EE 100%)",
    ...
    aiBorder:     dark ? "1px solid rgba(99,102,241,0.3)" : "1px solid rgba(255,255,255,0.6)",
...
Never say you can't do something — give a helpful answer always.`,
  ...
  const speakReply = (text) => {
    ...
    const pick   = voices.find(v => /samantha|karen|google uk|daniel/i.test(v.name))
    ...
    <div style={{ display:"flex", alignItems:"center", gap:3, height:20 }}>
      {[1,1.6,0.8,1.4,1,1.8,0.9].map((h, i) => (
        <div key={i} style={{
          ...
          background: accent,
          animation: `evDot ${0.6 + i * 0.07}s ease-in-out infinite`,
  ...
  return (
    <div style={{ position:"absolute", left:"50%", transform:"translateX(-50%)", top:7, zIndex:20 }}>
      ...
      <div style={{
        ...
        boxShadow: mode !== "idle"
          ? `0 0 0 3px ${accent}22, 0px 4px 20px rgba(99,102,241,0.12)`
          : "0px 4px 20px rgba(99,102,241,0.12)",
        transition:"all 0.3s",
        minWidth: 300,
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            boxShadow: mode !== "idle" ? `0 0 14px ${accent}88` : "0px 0px 12px rgba(99,102,241,0.5)",
        ...
        <div style={{
          ...
          animation: expanded ? "evFadeIn 0.25s ease" : "none",
        }}>
          {mode === "listening"
            ? <WaveBars />
            : (
              <span style={{
                fontFamily:"Inter", fontWeight: mode === "idle" ? 500 : 600,
                fontSize: mode === "speaking" ? 13 : 15,
                color: mode === "idle" ? t.aiText : mode === "error" ? "#EF4444" : t.textPrimary ?? "#1E1E1E",
                whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
                maxWidth: expanded ? 440 : 220,
        ...
        <div style={{ flexShrink:0 }}>
          {mode === "idle"    && <IdleDots />}
          {mode === "error"   && (
            <button onClick={reset} style={{ background:"none", border:"none", cursor:"pointer", fontSize:16, color:"#EF4444" }}>✕</button>
          )}
          {mode === "speaking" && (
            <button onClick={() => { synthRef.current?.cancel(); setMode("idle"); setExpanded(false); }}
              style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:accent, fontWeight:700 }}>
              Stop
            </button>
          )}
        </div>
      ...
      {/* ── reply bubble (when speaking, shows full text below pill) ── */}
      {mode === "speaking" && reply && (
        <div style={{
...
function TopBar({ theme }) {
  const t = tk(theme);
  return (
    <div style={{ position:"absolute", left:0, top:0, width:1280, height:62, background: t.topBarBg }}>
      {/* WiFi off */}
      <div style={{ position:"absolute", left:45, top:9, width:50, height:50, display:"flex", alignItems:"center", justifyContent:"center" }}>
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke={t.iconStroke} strokeWidth="2" strokeLinecap="round">
      ...
      <div style={{ position:"absolute", left:118, top:9, width:50, height:50, display:"flex", alignItems:"center", justifyContent:"center" }}>
        <svg width="30" height="36" viewBox="0 0 24 24" fill="none" stroke={t.iconStroke} strokeWidth="2.5" strokeLinecap="round">
          <path d="M6.5 6.5l11 11L12 23V1l5.5 5.5-11 11"/>
        </svg>
      </div>

      {/* EV Assistant — replaces the old static pill */}
      <EVAssistant theme={theme} />

      {/* Profile */}
      <div style={{ position:"absolute", left:1176, top:9, width:49, height:49, background: t.profileBg, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
...
function MapArea({ theme }) {
  ...
  return (
    <div style={{
      position: "absolute", width: 442, height: 593,
      left: 45, top: 62,
      boxShadow: t.shadow, borderRadius: 20, overflow: "hidden",
    }}>
      {/* Leaflet */}
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontFamily:"Inter", fontWeight:400, fontSize:40, lineHeight:"48px", color: t.textPrimary }}>750m</div>
          <div style={{ fontFamily:"Inter", fontWeight:400, fontSize:16, color: t.textPrimary }}>Turn Right</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily:"Inter", fontWeight:800, fontSize:48, lineHeight:"58px", color: t.textPrimary }}>80</div>
        <div style={{ fontFamily:"Inter", fontWeight:500, fontSize:14, color: t.textSecond }}>km/h</div>
      </div>
      ...
      <div style={{
        ...
        background: dark ? "#1C1F2A" : "#fff",
        boxShadow: t.shadow, borderRadius: 25,
        display: "flex", flexDirection: "column",
        alignItems: "center", justifyContent: "center",
      }}>
        <div style={{ fontFamily:"Inter", fontWeight:600, fontSize:16, color: t.textSecond }}>MAX</div>
        <div style={{ fontFamily:"Inter", fontWeight:700, fontSize:24, color: t.textPrimary }}>90</div>
      </div>
...
function BatteryCard({ onGoToSession, setActive, theme }) {
  ...
  const states = [
    ...
    { title:"Charging", percent:22, color:"#FF1E1E", subColor: theme === "dark" ? "#E8EAF0" : "#000", text:"95 km remaining", bottom:"Est. full charge in", eta:"1h 25m", charging:true, completed:false },
  ];
  ...
  return (
    <div onClick={nextState} style={{ position:"absolute", width:356, height:210, left:512, top:64, background: t.cardBg, border: t.cardBorder, boxShadow: t.shadow, borderRadius:20, cursor:"pointer", transition:"0.2s" }}>
      <div style={{ position:"absolute", left:30, top:22, fontFamily:"Inter", fontWeight:600, fontSize:24, color: t.textPrimary }}>{current.title}</div>
      <button onClick={() => setActive(1)} style={{ position:"absolute", right:22, top:10, background:"none", border:"none", fontSize:28, color: t.textPrimary, cursor:"pointer" }}>…</button>
      <div style={{ position:"absolute", left:30, top:55 }}>
        <svg width="120" height="60" viewBox="0 0 120 60">
          ...
          <rect x="8" y="14" width={(current.percent/100)*88} height="32" rx="6" fill={current.color} style={{ transition:"0.4s ease", filter: current.charging ? "drop-shadow(0 0 8px rgba(73,232,61,0.8))" : "none" }}/>
        </svg>
      </div>
      <div style={{ position:"absolute", left:199, top:65, fontFamily:"Inter", fontWeight:700, fontSize:36, color: t.textPrimary }}>{current.percent}%</div>

      {!current.completed && (
        <div style={{ position:"absolute", left:32, top:122, display:"flex", alignItems:"center", gap:8 }}>
          {current.charging
            ? <svg width="20" height="20" viewBox="0 0 24 24" fill="#49E83D"><path d="M13 2L4 14h6l-1 8 9-12h-6z"/></svg>
      ...
      {current.completed && (
        <div style={{ position:"absolute", left:32, top:122, display:"flex", alignItems:"center", gap:6 }}>
          ...
          <span style={{ fontFamily:"Inter", fontWeight:600, fontSize:15, color:"#49E83D" }}>Charge Completed</span>
        </div>
      )}
      ...
      {!current.completed && !current.charging && (
        <div style={{ position:"absolute", left:28, right:28, top:165, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
          ...
          <div onClick={(e) => { e.stopPropagation(); onGoToSession?.(); }} style={{ background: t.chargeBg, padding:"8px 14px", borderRadius:999, display:"flex", alignItems:"center", gap:6, cursor:"pointer", flexShrink:0 }}>
      ...
      {current.charging && (
        <div style={{ position:"absolute", left:32, top:166, display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ width:18, height:18, borderRadius:"50%", border:`2px solid ${t.divider}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color: t.textMuted }}>⏱</div>
          <span style={{ fontSize:14, color: t.textMuted, fontWeight:600 }}>{current.bottom}</span>
      ...
      {current.completed && (
        <div style={{ position:"absolute", left:28, right:28, top:162, display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div>
            <div style={{ fontSize:12, color: t.textSecond, fontWeight:600 }}>Last charge</div>
            <div style={{ fontSize:15, fontWeight:700, color: t.textPrimary }}>100%</div>
          </div>
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:15, fontWeight:700, color: t.textSecond }}>3h 20m</div>
            <div style={{ fontSize:12, color: t.textMuted, fontWeight:600 }}>Today, 09:15AM</div>
          </div>
        </div>
      )}
    </div>
...
function ClimateCard({ temp, setTemp, setActive, theme }) {
  ...
  const toggleBar = (index) => { if (index === 0) return; setActiveBars((prev) => prev.map((v,i) => i === index ? !v : v)); };
  const bars = [10, 14, 18];

  return (
    <div style={{ position:"absolute", width:356, height:142, left:512, top:300, background: t.cardBg, border: t.cardBorder, boxShadow: t.shadow, borderRadius:20 }}>
      <div style={{ position:"absolute", left:28, top:10, fontFamily:"Inter", fontWeight:600, fontSize:24, color: t.textPrimary }}>Climate</div>
      <button onClick={() => setActive(3)} style={{ position:"absolute", right:20, top:5, background:"none", border:"none", fontFamily:"Inter", fontSize:32, color: t.textPrimary, cursor:"pointer" }}>›</button>
      <div style={{ position:"absolute", left:31, top:51, fontFamily:"Inter", fontWeight:500, fontSize:16, color: t.textSecond }}>Auto</div>
      <div style={{ position:"absolute", left:26, top:78, width:80, height:46, background: t.inputBg, border: t.inputBorder, borderRadius:20, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
        ...
        <div style={{ display:"flex", gap:3, alignItems:"flex-end" }}>
          {bars.map((h,i) => (
            <div key={i} onClick={() => toggleBar(i)} style={{ width:4, height:h, borderRadius:2, cursor: i===0 ? "default" : "pointer", background: activeBars[i] ? "#0096FF" : t.divider, transition:"0.2s" }}/>
          ))}
        </div>
      </div>
      <div style={{ position:"absolute", right:20, top:72, display:"flex", alignItems:"center", gap:10 }}>
        <button onClick={() => setTemp(t2 => Math.max(16, t2-1))} style={{ width:44, height:44, border: t.inputBorder, borderRadius:20, background: t.inputBg, fontSize:20, cursor:"pointer", color: t.textPrimary }}>−</button>
        <span style={{ fontFamily:"Inter", fontWeight:500, fontSize:20, minWidth:50, textAlign:"center", color: t.textPrimary }}>{temp}°</span>
        <button onClick={() => setTemp(t2 => Math.min(30, t2+1))} style={{ width:44, height:44, border: t.inputBorder, borderRadius:20, background: t.inputBg, fontSize:20, cursor:"pointer", color: t.textPrimary }}>+</button>
      </div>
    </div>
  );
}
...
function TripCard({ setActive, theme }) {
  const t = tk(theme);
  return (
    <div style={{ position:"absolute", width:356, height:180, left:512, top:470, background: t.cardBg, border: t.cardBorder, boxShadow: t.shadow, borderRadius:20 }}>
      <div style={{ position:"absolute", width:40, height:40, left:21, top:8, background: t.tripAccentBg, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ position:"absolute", width:40, height:40, left:21, top:57, background: theme === "dark" ? "rgba(16,185,129,0.15)" : "#E8F9EF", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2" strokeLinecap="round">
          <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
          <circle cx="12" cy="9" r="2.5"/>
        </svg>
      </div>
      <div style={{ position:"absolute", left:74, top:19, fontFamily:"Inter", fontWeight:500, fontSize:15, color: t.textPrimary }}>Trip</div>
      <div style={{ position:"absolute", left:74, top:60, fontFamily:"Inter", fontWeight:500, fontSize:14, color: t.textPrimary }}>Vivacity Mall</div>
      <div style={{ position:"absolute", left:74, top:78, fontFamily:"Inter", fontWeight:500, fontSize:12, color: t.textSecond }}>Jalan Setia Raja</div>
      <button onClick={() => setActive(2)} style={{ position:"absolute", right:18, top:16, background:"none", border:"none", fontFamily:"Inter", fontSize:22, cursor:"pointer", color: t.textPrimary }}>›</button>
      {[
        ...
        <div key={label} style={{ position:"absolute", width:92, height:56, left, top:107, background: accent ? t.tripAccentBg : t.tripBadgeBg, border: t.tripBadgeBorder, boxShadow: t.shadow, borderRadius:16 }}>
          <div style={{ position:"absolute", left:14, top:8,  fontFamily:"Inter", fontWeight:500, fontSize:10, color: accent ? "#6366F1" : t.textMuted }}>{label}</div>
          <div style={{ position:"absolute", left:14, top:20, fontFamily:"Inter", fontWeight:700, fontSize:15, color:"#6366F1" }}>{value}</div>
          <div style={{ position:"absolute", left:14, top:38, fontFamily:"Inter", fontWeight:500, fontSize:10, color: t.textMuted }}>{sub}</div>
        </div>
      ))}
    </div>
  );
...
function ClockCard({ theme }) {
  ...
  return (
    <div style={{ position:"absolute", width:332, height:164, left:897, top:64, background: t.cardBg, border: t.cardBorder, boxShadow: t.shadow, borderRadius:20 }}>
      <div style={{ position:"absolute", left:33, top:20, fontFamily:"Roboto", fontWeight:500, fontSize:14, color:"#FFA500" }}>GOOD MORNING</div>
      <div style={{ position:"absolute", left:33, top:50, fontFamily:"Inter", fontWeight:700, fontSize:32, color: t.textPrimary }}>{String(hh).padStart(2,"0")}:{mm}</div>
      ...
      <div style={{ position:"absolute", right:20, top:20 }}>
        <svg width="52" height="52" viewBox="0 0 52 52">
          <circle cx="26" cy="26" r="22" fill={theme === "dark" ? "#2C2F3E" : "#D9D9D9"} opacity="0.5"/>
      ...
      <div style={{ position:"absolute", left:33, top:94, fontFamily:"Roboto", fontWeight:500, fontSize:14, color: t.textPrimary }}>
      ...
      <div style={{ position:"absolute", left:33, top:120, display:"flex", gap:14 }}>
        {days.map((d,i) => (
          <div key={i} style={{ width:25, height:27, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, border: i===dow ? "2px solid #0078FF" : "none", background: i===dow ? (theme === "dark" ? "rgba(0,120,255,0.15)" : "rgba(245,245,245,0.5)") : "transparent", fontFamily:"Roboto", fontWeight:500, fontSize:14, color: i===dow ? "#0078FF" : t.textMuted }}>
            {d}
          </div>
        ))}
      </div>
...
function WeatherCard({ theme }) {
  const t = tk(theme);
  return (
    <div style={{ position:"absolute", width:332, height:138, left:896, top:254, background: theme === "dark" ? "#1C1F2A" : "linear-gradient(135deg,#e0f2fe 0%,#f0fdf4 100%)", border: t.cardBorder, boxShadow: t.shadow, borderRadius:20, overflow:"hidden" }}>
      <img src="/weather.png" alt="Map" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center 25%", opacity: theme === "dark" ? 0.4 : 1 }}/>
      <div style={{ position:"absolute", left:20, top:16, fontFamily:"Inter", fontWeight:600, fontSize:20, color: t.textPrimary }}>Weather</div>
      <div style={{ position:"absolute", right:20, top:18, fontFamily:"Inter", fontWeight:600, fontSize:40, color: t.textPrimary }}>24<span style={{ fontSize:24, fontWeight:700 }}>°C</span></div>
      <div style={{ position:"absolute", right:20, top:56, fontFamily:"Inter", fontWeight:500, fontSize:16, color: t.textSecond }}>Cloudy</div>
      <div style={{ position:"absolute", right:20, top:80, fontFamily:"Inter", fontWeight:500, fontSize:12, color: t.textSecond }}>20°C / 26°C</div>
    </div>
  );
}
...
function MusicCard({ theme }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (playing) setProgress((p) => { if (p >= total) { nextSong(); return 0; } return p + 1; });
    }, 1000);
  ...
  return (
    <div style={{ position:"absolute", width:334, height:229, left:895, top:425, background:"linear-gradient(134.6deg,#6A8DFF 1.6%,#B8A6FF 50%)", borderRadius:20, overflow:"hidden", boxShadow:"0px 6px 20px rgba(0,0,0,0.1)" }}>
      <div style={{ position:"absolute", left:26, top:15, fontFamily:"Inter", fontWeight:400, fontSize:14, color:"rgba(255,255,255,0.7)" }}>Now Playing</div>
...
/* ── Page Root ───────────────────────────────────────────────────── */
export default function HomePage({ navActive, setNavActive, onGoToSession, theme }) {
  const [temp, setTemp] = useState(24);
  const t = tk(theme);
  return (
    <div style={{ position:"relative", width:1280, height:800, background: t.pageBg, overflow:"hidden", fontFamily:"Inter, sans-serif", transition:"background 0.3s" }}>
      <TopBar theme={theme} />
      <MapArea theme={theme} />
